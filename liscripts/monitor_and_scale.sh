...
do
	...
	ant elastic-controller -Dproject=ycsb -DtWindow=15 -DnumPart=12  -DplannerID=1 -Dprovisioning=0 -DtimeLimit=5000 -Dglobal.hasher_plan=next_round.json -Dmonitoring=0 -DsitesPerHost=1 -DpartPerSite=$((12 / num_all_nodes)) -DhighCPU=160 -DlowCPU=110 -DchangeParts="+9;+10;+11" | tee $folder/controller_scale.out && cp plan_out.json next_round.json && ./liscripts/copy_to_all.sh "`cat ./nodes`" ./next_round.json
        break
    fi
